 ...
 *
 * <p>ActionChain implements fail-fast execution: if any action in the chain returns
 * {@code false}, execution stops immediately and the entire chain is considered failed.
 * Only if all actions succeed does the chain succeed.</p>
 *
 * <h2>Usage</h2>
 * <p>ActionChains are typically created implicitly when chaining actions:</p>
 * <pre>{@code
 * bank().open()
 ...
 * <ul>
 *   <li><strong>Sequential:</strong> Actions execute in the order they were added</li>
 *   <li><strong>Fail-fast:</strong> First action that returns {@code false} stops the chain</li>
 *   <li><strong>All-or-nothing:</strong> Chain succeeds only if every action succeeds</li>
 * </ul>
 *
 * <h2>Wait Operations</h2>
 * <p>Chains can include wait conditions that pause execution until a condition becomes true:</p>
 * <pre>{@code
 * inventory().drop("Logs")
...
public class ActionChain implements Action {
     ...
     *
     * <p>The new action will only execute if all previous actions in the chain
     * have succeeded. If any previous action failed, this action will not execute.</p>
     *
     ...
     */
    public ActionChain waitUntil(BooleanSupplier condition, LongSupplier pollingRateSupplier, long timeoutMs) {
     ...
     *
     * <p>Uses a default timeout of 5 seconds.</p>
     *
     * @param condition The condition to wait for. Must not be null.
     * @param pollingRateSupplier Supplier that returns wait time between checks in milliseconds
     * @return This ActionChain for continued chaining
     ...
     * timeout (5 seconds) is reached. Uses a default polling rate of 100ms.</p>
     *
     * @param condition The condition to wait for. Must not be null.
     * @return This ActionChain for continued chaining
     ...
     *
     * <p>Execution will pause until the condition becomes {@code true} or the
     * specified timeout is reached. Uses a default polling rate of 100ms.</p>
     *
     * @param condition The condition to wait for. Must not be null.
     * @param timeoutMs Maximum time to wait in milliseconds. Must be positive.
     * @return This ActionChain for continued chaining
     ...
     *
     * <p>This is a convenience method that uses a fixed polling rate.</p>
     *
     * @param condition The condition to wait for. Must not be null.
     ...
     * @param pollingRateSupplier Supplier that returns the wait time in milliseconds for each iteration
     * @param timeoutMs Maximum time to repeat before giving up, in milliseconds
     * @return {@code true} if the exit condition was met, {@code false} if timeout or action failure
     */
    public ActionChain repeatUntil(BooleanSupplier exitCondition, LongSupplier pollingRateSupplier, long timeoutMs) {
        return then(() -> TimingUtils.repeatUntil(this::execute, exitCondition, pollingRateSupplier, timeoutMs));
    }
     ...
     * @return {@code true} if the exit condition was met, {@code false} if timeout or action failure
     */
    public ActionChain repeatUntil(BooleanSupplier exitCondition, long pollingRateMs, long timeoutMs) {
        return this.repeatUntil(exitCondition, () -> pollingRateMs, timeoutMs);
    }

    /**
     * Repeats the entire action chain until a condition becomes true with dynamic polling rate and default timeout.
     *
     * <p>Uses a default timeout of 30 seconds.</p>
     *
     * @param exitCondition Condition that stops the loop when it becomes {@code true}
     * @param pollingRateSupplier Supplier that returns the wait time in milliseconds for each iteration
     * @return {@code true} if the exit condition was met, {@code false} if timeout or action failure
     */
    public ActionChain repeatUntil(BooleanSupplier exitCondition, LongSupplier pollingRateSupplier) {
     ...
     *
     * <p>Actions are executed in the order they were added to the chain. If any
     * action returns {@code false}, execution stops immediately and this method
     * returns {@code false}. Only if all actions return {@code true} does this
     * method return {@code true}.</p>
     *
     * <p><strong>Note:</strong> In the fluent API, this method is typically called
     * automatically by the situation framework. Manual calls are rarely needed.</p>
     *
     * @return {@code true} if all actions in the chain succeeded, {@code false} if
     *         any action failed or if the chain is empty
     */
    @Override
